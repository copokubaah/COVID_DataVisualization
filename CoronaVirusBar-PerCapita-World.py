...
class Create_Animation():
    ...
    def plotbargraph(self, title, rect, bgimg, adnice):
        ...
        for i, cntry in enumerate(self.ynames):
            ...
            if cntry == 'Newcastle Utd' and self.plot_dict['bkgcolor'][0] < 0.5:
                colorname = [1, 1, 1, coloralpha]
    ...
    def add_text(self, sidetop, rect1=[0.84, 0, 0.15, 0.85]):
        ...
        #               verticalalignment='center', horizontalalignment='center', color=self.headcolor, fontsize=7,
        #               fontname=self.plot_dict['myfont'])
        # self.ax1.text(xs, 0.685, f'(% of World Total)', transform=self.ax1.transAxes,
        #               verticalalignment='center', horizontalalignment='center', color=self.headcolor, fontsize=5,
        #               fontweight='semibold',
        #               fontname=self.plot_dict['myfont'])
        ...
        for n, idx, i in zip(np.arange(len(contname)), self.indcont[:, self.num], self.smothcont[:, self.num]):
            ...
            self.ax1.text(xs, ys, '{}'.format(cntry.upper()), transform=self.ax1.transAxes, fontweight='semibold',
                          ...
                          fontname=self.plot_dict['myfont'])
            self.ax1.text(xs, ys - ystp, '{}\n{:2.1f} per 1M pop'.format(text1, tnum), fontweight='semibold',
        ...
        self.ax1.text(xs, ys, 'WORLD', transform=self.ax1.transAxes,
                      va='center', ha='center', color=self.headcolor, fontweight='semibold', fontsize=bigfont + 2,
                      fontname=self.plot_dict['myfont'])
        self.ax1.text(xs, ys - ystp*1.2, '{}\n{:2.1f} per 1M pop'.format(text1, tworld), transform=self.ax1.transAxes, va='center', ha='center',
                      color=self.headcolor, fontsize=smallfont + 2, fontweight='semibold', fontname=self.plot_dict['myfont'])
    ...
    def add_report(self, report):
        ...
        self.ax2 = self.fig.add_axes([0.5, 0.15, 0.2, 0.2])
        offset_image2(0.5, 0.5, corimg, self.ax2, 0.15, 500, 500)
        degrees = np.linspace(0, 360, 5 * self.intpolnum)
        rotated_img = ndimage.rotate(corimg, degrees[int(self.num % (5 * self.intpolnum))])
        offset_image2(0.5, 0.5, rotated_img, self.ax2, 0.12, 500, 500)
...
def plotgraph(num):
    ...
    adnice = 0
    k.plotbargraph(title, rect, bgimg, adnice)
    zoomscalar = 0.018
